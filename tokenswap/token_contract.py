...
class Token_X_core(sp.Contract):
    ...
    def transfer(self, params):
        ...
        sp.verify(self.is_administrator(sp.source) |
            (~self.is_paused() &
                ((params.from_ == sp.source) |
                 (self.data.balances[params.from_].approvals[sp.source] >= params.value))))
        self.addAddressIfNecessary(params.to_)
        sp.verify(self.data.balances[params.from_].balance >= params.value)
        self.data.balances[params.from_].balance = sp.as_nat(self.data.balances[params.from_].balance - params.value)
        self.data.balances[params.to_].balance += params.value
        sp.if (params.from_ != sp.source) & (~self.is_administrator(sp.source)):
            self.data.balances[params.from_].approvals[sp.source] = sp.as_nat(self.data.balances[params.from_].approvals[sp.source] - params.value)

    @sp.entry_point
    def approve(self, params):
        sp.set_type(params, sp.TRecord(spender = sp.TAddress, value = sp.TNat).layout(("spender", "value")))
        sp.verify(~self.is_paused())
        alreadyApproved = self.data.balances[sp.source].approvals.get(params.spender, 0)
        sp.verify((alreadyApproved == 0) | (params.value == 0), "UnsafeAllowanceChange")
        self.data.balances[sp.source].approvals[params.spender] = params.value
...
if "templates" not in __name__:
    ...
    def test():
        ...
        c1 = Token_X(sp.address("tz1a21nx5BggGmn4qr8QBPtZzKw6fs36isJY"))
